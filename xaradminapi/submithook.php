...
function formantibot_adminapi_submithook($args)
{
    ...
    if ($usecaptcha != 1)  {
        ...
        if ($captchatype ==1) {
            ...
            $privatekey = trim($settings['captchaprivate']); // you got this from the signup page
            $remoteadd = xarServerGetVar('REMOTE_ADDR');
            if(!xarVarFetch('recaptcha_challenge_field',  'str', $recaptcha_challenge_field, '',XARVAR_DONT_SET)) {return;}
            if(!xarVarFetch('recaptcha_response_field',  'str', $recaptcha_response_field, '',XARVAR_DONT_SET)) {return;}
